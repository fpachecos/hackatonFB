 ...
 * @version $Id: SettlementFinancialAdjustmentServiceImpl.java 68060 2016-07-12 14:56:16Z roanbr $
 */
@Stateless
public class SettlementFinancialAdjustmentServiceImpl
    implements SettlementFinancialAdjustmentService {
  ...
  @Override
  public SettlementFinancialAdjustment generateAdjustment(
    ...
    return this.createAdjustmentByFinancialAdjustment(financialAdjustment, priorityProduc,
        currencyCode);
  }

  private SettlementFinancialAdjustment createAdjustmentByFinancialAdjustment(
      final SettlementFinancialAdjustment financialAdjustment,
      final ContractedProduct priorityProduct, final Integer codCurrency) throws BusinessException {
    ...
    if (financialAdjustment.getCdProduct() == null || financialAdjustment.getCdProduct() == 0) {
      ...
      Product lSelectedProduct =
          this.adjustPaymentTypeService.getAdjustmentProduct(filterBuilder.build());
              ...
              this.hashService
                  .generateHashSHA1(
                      adjustment.getRequestNumber()
                          + this.hashService
                              .generateGroupingKey(
                                  new HashFieldsBuilder()
                                      .settlementDate(adjustment.getSettlementAdjstmentDate())
                                      .batchDate(adjustment.getBatchDate())
                                      .loadFileId(
                                          String.valueOf(financialAdjustment.getNuLoadFileId()))
                                      .customerNumber(adjustment.getCustomerNumber())
                                      .customerModNumber(adjustment.getCustomerModNumber())
                                      .installmentSegment(0)
                                      .productCode(adjustment.getProductCode())
                                      .movementType(adjustment.getMovementTypeCode().getCode())
                                      .fundingCurrency(codCurrency)
                                      .installmentSequenceNumber(NON_INSTALLMENT_SEQUENCE_NUMBER)
                                      .build())));
  ...
  private void validateAdjustmentDate(final SettlementFinancialAdjustment financialAdjustment) {
    ...
    Date settlementDate = this.settlementService
        .adjustSettlementDate(financialAdjustment.getDtSettlementAdjustment(), settlementType);
  ...
  @Override
  public SettlementMovement generateMovement(final SettlementAdjustment adjustment)
      throws BusinessException {
    Integer currencyCode = this.settlementService
        .getCodCurrencyByCustomerNumber(adjustment.getCustomerNumber().toString());
    final SettlementMovement movement =
        this.validation.createMovementByAdjustment(adjustment, currencyCode);
