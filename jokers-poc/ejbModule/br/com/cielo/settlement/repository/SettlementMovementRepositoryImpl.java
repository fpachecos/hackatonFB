 ...
 * @version $Id: SettlementMovementRepositoryImpl.java 55522 2016-01-19 11:47:17Z waldeir $
 */
@Stateless
public class SettlementMovementRepositoryImpl implements SettlementMovementRepository {
  ...
  @Override
  public void insertSettlementFinancialMovement(
      SettlementFinancialMovement settlementFinancialMovement) {
    Query query =
        entityManager.createNativeQuery(SettlementMovementRepository.INSERT_SETTLEMENT_MOVEMENT);
    new SettlementFinancialMovementParamProvider().applyParameters(settlementFinancialMovement,
        query);
    query.executeUpdate();
  }
  ...
  public void insertMovementRelAdjustment(SettlementFinancialMovement settlementFinancialMovement) {
    Query query = entityManager
        .createNativeQuery(SettlementMovementRepository.INSERT_SETTLEMENT_REL_MOVEMENT_ADJUSTMENT);
    int index = 1;
    ...
    query.setParameter(index++,
        settlementFinancialMovement.getSettlementMovement().getSettlementDate());
  ...
  @Override
  public void insertMovementFinancialComplement(
      SettlementFinancialMovement settlementFinancialMovement) {
    Query query = entityManager
        .createNativeQuery(SettlementMovementRepository.INSERT_SETTLEMENT_MOVEMENT_COMPLEMENT);
    new MovementCompanyComplementParamProvider().applyParameters(settlementFinancialMovement,
        query);
    query.executeUpdate();
  }
}
