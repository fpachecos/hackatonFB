	...
	public SettlementFinancialAdjustment(List<String> listaCampos) throws ParseException {
		for (int i = 0; i < listaCampos.size(); i++) {
			...
			case CD_REJECT_REASON:
				this.setCdRejectReason(listaCampos.get(i).isEmpty() ? null : Integer.parseInt(listaCampos.get(i)));
				break;
			case DT_SETTLEMENT_ADJUSTMENT:
				this.setDtSettlementAdjustment(
						listaCampos.get(i).isEmpty() ? null : formatterDiaMesAnoResumido.parse(listaCampos.get(i)));
				break;
			case VL_GROSS:
			...
			case CD_STATUS_REQUEST:
				...
				break;
			case NU_CUSTOMER:
				this.setNuCustomer(listaCampos.get(i).isEmpty() ? null : Long.valueOf(listaCampos.get(i)));
				break;
			case NU_MOD_CUSTOMER:
				this.setNuModCustomer(listaCampos.get(i).isEmpty() ? null : Integer.valueOf(listaCampos.get(i)));
				break;
			case NU_BATCH:
			...
			case CD_ADJUSTMENT_REASON:
				this.setCdAdjustentReason(listaCampos.get(i).isEmpty() ? null : Integer.valueOf(listaCampos.get(i)));
				break;
			...
			case NU_LOAD_FILE_ID:
				...
				break;
			case CD_PRODUCT:
				this.setCdProduct(listaCampos.get(i).isEmpty() ? null : Integer.valueOf(listaCampos.get(i)));
				break;
			case CD_ENTRY_TYPE:
				this.setEntryType(listaCampos.get(i).isEmpty() ? null : Integer.parseInt(listaCampos.get(i)));
				break;
			default:
				break;
			}
